...
/**
 * 动态加载PHP文件
 * @staticvar array $_file 缓存加装过的文件
 * @param string $file 需要加载的文件
 * @param boolean $replace 是否允许重复加载
 * @return boolean 返回加载的文件 
 */
function load($file, $replace = false) {
	static $_file = array();
	if (($replace || !isset($_file[$file])) && file_exists($file)) {
		return require $file;
	} else {
		return false;
	}
